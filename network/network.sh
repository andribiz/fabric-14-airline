...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-boeing.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-airbus.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
...
## Bring up the network if it is not arleady up.
 ...
 scripts/createChannel.sh $CHANNEL_NAME $CHANNEL_PROFILE $CLI_DELAY $MAX_RETRY $VERBOSE
  if [ $? -ne 0 ]; then
